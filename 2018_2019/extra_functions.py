...
def real_round_from_line(line):
	...
	letters = {'A': 'Ciolle United',
	           ...
	           'F': 'Ac Picchia',
	           'G': 'FC BOMBAGALLO',
	           'H': 'Atletico cu tri Arancini'}

	final_round = [letters[i] for i in list(line)]
	final_round = grouper(final_round, 2)

	return list(grouper(final_round, len(letters) // 2))
...
def random_rounds(number):
	...
	myfile = os.getcwd() + '/All_Leagues_8teams.txt'
