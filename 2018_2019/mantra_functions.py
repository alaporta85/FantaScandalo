...
def check_when_0_subst(day, fantateam, players_in_field):
	...
	scheme = dbf.db_select(
			...
			where='team_name = "{}"'.format(fantateam))[0]

	scheme_details = dbf.db_select(
			table='schemes_details',
			columns=['details'],
			where='scheme = "{}"'.format(scheme))[0].split(', ')[1:]
	...
	for i in range(len(players_in_field)):
		...
		# else, a role with malus
			...
			rl_malus = dbf.db_select(
					table='malus',
					columns=['malus'],
					where='role = "{}"'.format(role_to_cover[0]))[0]
...
def player_vote(day, player_name, source):
	...
	vote = dbf.db_select(
		    table='votes',
            columns=[source],
            where='day={} AND name="{}"'.format(day, player_name))
...
def select_lineup(day, fantateam):
	...
	lineup = dbf.db_select(
			table='lineups',
			columns=['day_{}'.format(day)],
			where='team_name = "{}"'.format(fantateam))[0].split(', ')
...
def save_mantra_lineup(day, fantateam, result):
	...
	result = ["{}:{}".format(nm, rl) for nm, rl in result]
	dbf.db_update(
			table='mantra_lineups',
			columns=['day_{}'.format(day)],
			values=[', '.join(result)],
			where='team_name="{}"'.format(fantateam))


START_PLAYERS = 10
# DAY = 2
# FANTATEAM = 'fcpastaboy'
#
# print(mantra(DAY, FANTATEAM, START_PLAYERS, source='alvin', roles=True))
